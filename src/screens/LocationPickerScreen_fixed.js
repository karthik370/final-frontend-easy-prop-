...
const LocationPickerScreen = ({ route, navigation }) => {
  ...
  const getMyLocation = async () => {
    ...
    try {
      ...
      try {
        ...
        if ((Math.abs(latitude - 37.42) < 0.5 && Math.abs(longitude - (-122.08)) < 0.5) ||
            (Math.abs(latitude - 37.78) < 0.5 && Math.abs(longitude - (-122.4)) < 0.5)) {
          console.log('Detected mock location from emulator, using default India location instead');
          useDefaultIndiaLocation();
          setIsLoading(false);
          return;
        }
        ...
        try {
          ...
          console.log('Balanced accuracy failed too, using default India location:', balancedError);
          useDefaultIndiaLocation();
          setIsLoading(false);
          return;
        }
  ...
  const saveLocation = () => {
    ...
    if (typeof onSelectLocation === 'function') {
      // Use a serializable version of the location data to avoid navigation warnings
